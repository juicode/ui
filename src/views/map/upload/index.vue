      ...
      <el-row :gutter="10" class="mb8">
        ...
        <el-col :span="1.5">
          <el-button
            ...
            :disabled="multiple"
            @click="handleDelete"
            v-hasPermi="['map:realtime:remove']"
          >删除</el-button>
        </el-col>
        <el-col :span="1.5">
          <el-button
  ...
  export default {
    ...
    methods: {
        ...
        this.$refs["form"].validate(valid => {
          if (valid) {
            if (this.form.id != null) {
              updateRealtime(this.form).then(response => {
      ...
      handleDelete(row) {
        ...
        this.$modal.confirm('是否确认删除check编号为"' + ids + '"的数据项？').then(function() {
          return delRealtime(ids);
        }).then(() => {
          this.getList();
      ...
      /** 导出按钮操作 */
      handleExport() {
        this.download('map/realtime/export', {
          ...this.queryParams
